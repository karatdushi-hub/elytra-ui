...
function Library:CreateWindow(Settings: { Title: string, Size: UDim2, Transparency: number, MinimizeKeybind: Enum.KeyCode?, Blurring: boolean, Theme: string, HubName: string?, GameName: string?, HubDescription: string? })
	...
	ElytraConnect(MinimizeIcon.MouseEnter, function()
		Tween(MinimizeIcon, .2, { BackgroundTransparency = 0 })
	end)

	ElytraConnect(MinimizeIcon.MouseLeave, function()
		Tween(MinimizeIcon, .2, { BackgroundTransparency = 0.2 })
	end)
	...
	function Options:AddToggle(Settings: { Title: string, Description: string, Default: boolean, Tab: Instance, Callback: any })
		...
		-- Store toggle reference for external control
		if not Options.Toggles then
			Options.Toggles = {}
		end
		Options.Toggles[Settings.Title] = {
			Toggle = Toggle,
			Set = Set,
			On = On,
	...
	--// Method to control toggle externally
	function Options:SetToggle(ToggleName, Value)
		if Options.Toggles and Options.Toggles[ToggleName] then
			local ToggleData = Options.Toggles[ToggleName]
			ToggleData.Set(Value)
			ToggleData.Callback(Value)
		end
	end

	--// Method to get toggle state
	function Options:GetToggle(ToggleName)
		if Options.Toggles and Options.Toggles[ToggleName] then
			return Options.Toggles[ToggleName].On.Value
		end
		return nil
	end
	...
	function Options:AddKeybind(Settings: { Title: string, Description: string, Tab: Instance, Callback: any })
		...
		DotIndicator.Position = UDim2.new(0.5, -3, 0.5, -3)
